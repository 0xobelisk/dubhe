...
module card_duel::deck {
    ...
    const TABLE_NAME: vector<u8> = b"deck";
    ...
    public(package) fun delete(user_storage: &mut UserStorage, ctx: &TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        dapp_system::delete_record<DappKey>(dapp_key::new(), user_storage, key_tuple, vector[b"card_ids"], ctx);
    }

    public fun get(user_storage: &UserStorage): vector<address> {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let card_ids_raw = dapp_system::get_field<DappKey>(user_storage, key_tuple, b"card_ids");
        let mut card_ids_bcs = sui::bcs::new(card_ids_raw);
        let value = sui::bcs::peel_vec_address(&mut card_ids_bcs);
        value
    }

    public(package) fun set(user_storage: &mut UserStorage, value: vector<address>, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let field_names = vector[b"card_ids"];
