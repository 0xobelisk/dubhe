...
module ville::genesis {
    ...
    public fun run(dapp_hub: &mut DappHub, clock: &Clock, ctx: &mut TxContext) {
        ...
        let mut ds = dapp_system::create_dapp(dapp_key, dapp_hub, string(b"ville"), string(b"Ville - Full-chain AI agent town built with Dubhe"), 1, clock, ctx);

        // Set up initial DApp state (e.g. default resource values).
        ville::deploy_hook::run(&mut ds, ctx);
