...
module ville::gold {
    ...
    // ─── transferable: User ↔ BuildingStorage (fungible) ─────────────
    public(package) fun transfer_user_to_building(
        user:   &mut UserStorage,
        target: &mut dubhe::dapp_service::ObjectStorage<ville::building::Building>,
        amount: u64,
        ctx:    &mut TxContext,
    ) {
        sub(user, amount, ctx);
        ville::building::add_gold(target, amount);
    }

    public(package) fun transfer_building_to_user(
        source: &mut dubhe::dapp_service::ObjectStorage<ville::building::Building>,
        user:   &mut UserStorage,
        amount: u64,
        ctx:    &mut TxContext,
    ) {
        ville::building::sub_gold(source, amount);
        add(user, amount, ctx);
    }
