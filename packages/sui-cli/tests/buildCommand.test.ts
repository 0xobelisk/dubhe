...
describe('removeEnvFromMoveLock', () => {
  let tempDir: string;
  let moveLockPath: string;

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'removeenv-test-'));
    moveLockPath = path.join(tempDir, 'Move.lock');
  });
  ...
  it('strips the target [env.*] section and leaves pinned sections intact', async () => {
    const content = `# Generated by move; do not edit
# This file should be checked in.
...
[env.mainnet]
chain-id = "35834a8a"
original-published-id = "0x1111"
latest-published-id = "0x1111"
published-version = "1"
`;
    fs.writeFileSync(moveLockPath, content, 'utf-8');

    const { removeEnvFromMoveLock } = await import('../src/utils/utils');
    removeEnvFromMoveLock(moveLockPath, 'mainnet');

    const result = fs.readFileSync(moveLockPath, 'utf-8');
    expect(result).not.toContain('[env.mainnet]');
    expect(result).not.toContain('original-published-id = "0x1111"');
    expect(result).toContain('[pinned.mainnet.Dubhe]');
    expect(result).toContain('[pinned.testnet.Dubhe]');
  });

  it('only removes the specified network, leaving other [env.*] sections untouched', async () => {
    const content = `[move]
version = 4
...
[env.testnet]
chain-id = "4c78adac"
original-published-id = "0x2222"
latest-published-id = "0x2222"
published-version = "1"
`;
    fs.writeFileSync(moveLockPath, content, 'utf-8');
    ...
    const result = fs.readFileSync(moveLockPath, 'utf-8');
    expect(result).not.toContain('[env.mainnet]');
    expect(result).toContain('[env.testnet]');
    expect(result).toContain('0x2222');
  });

  it('is a no-op when the file does not exist', async () => {
    const { removeEnvFromMoveLock } = await import('../src/utils/utils');
    // Should not throw
    expect(() => removeEnvFromMoveLock('/nonexistent/Move.lock', 'mainnet')).not.toThrow();
  });

  it('is a no-op when the target [env.*] section is absent', async () => {
    const content = `[move]
version = 4
