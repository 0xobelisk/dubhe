...
module guild::genesis {
    ...
    public fun run(dapp_hub: &mut DappHub, clock: &Clock, ctx: &mut TxContext) {
        ...
        let mut ds = dapp_system::create_dapp(dapp_key, dapp_hub, string(b"guild"), string(b"Guild game — codegen annotation regression"), 1, clock, ctx);

        // Set up initial DApp state (e.g. default resource values).
        guild::deploy_hook::run(&mut ds, ctx);
