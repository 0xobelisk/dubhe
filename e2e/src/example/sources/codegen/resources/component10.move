...
module example::component10 {
    ...
    public(package) fun delete(user_storage: &mut UserStorage, player: address, ctx: &TxContext) {
        ...
        key_tuple.push_back(TABLE_NAME);
        key_tuple.push_back(to_bytes(&player));
        dapp_system::delete_record<DappKey>(dapp_key::new(), user_storage, key_tuple, vector[b"direction"], ctx);
    }

    public fun get(user_storage: &UserStorage, player: address): Direction {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        key_tuple.push_back(to_bytes(&player));
        ...
        let value = example::direction::decode(&mut direction_bcs);
        value
    }

    public(package) fun set(user_storage: &mut UserStorage, player: address, value: Direction, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        key_tuple.push_back(to_bytes(&player));
        let field_names = vector[b"direction"];
    ...
    public(package) fun mint(
        ...
        player: address, direction: Direction,
        ctx: &mut TxContext,
    ) {
        ensure_has_not(user_storage, player);
        set(user_storage, player, direction, ctx);
    }
}