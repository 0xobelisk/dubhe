...
module example::component29 {
    ...
    public fun get(user_storage: &UserStorage): (vector<address>) {
        ...
        let value = sui::bcs::peel_vec_address(&mut value_bcs);
        (value)
    }

    public(package) fun set(dapp_hub: &DappHub, user_storage: &mut UserStorage, value: vector<address>, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
