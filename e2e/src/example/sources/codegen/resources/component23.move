  ...
  module example::component23 { 
    ...
    public fun get(user_storage: &UserStorage): (vector<u8>) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let value_raw = dapp_system::get_field<DappKey>(user_storage, key_tuple, b"value");
        let mut value_bcs = sui::bcs::new(value_raw);
        let value = sui::bcs::peel_vec_u8(&mut value_bcs);
        (value)
    }

    public(package) fun set(dapp_hub: &DappHub, user_storage: &mut UserStorage, value: vector<u8>, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
