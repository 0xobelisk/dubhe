...
module example::component19 {
    ...
    public fun get(user_storage: &UserStorage): (u128) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let value_raw = dapp_system::get_field<DappKey>(user_storage, key_tuple, b"value");
        let mut value_bcs = sui::bcs::new(value_raw);
        let value = sui::bcs::peel_u128(&mut value_bcs);
        (value)
    }

    public(package) fun set(dapp_hub: &DappHub, user_storage: &mut UserStorage, value: u128, ctx: &mut TxContext) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
