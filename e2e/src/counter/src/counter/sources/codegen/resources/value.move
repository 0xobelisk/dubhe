  ...
  module counter::value { 
    ...
    public(package) fun delete(dapp_storage: &mut DappStorage) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        dapp_system::delete_global_field<DappKey>(dapp_storage, key_tuple, b"value");
        dapp_system::delete_global_record<DappKey>(dapp_storage, key_tuple);
    }

    public fun get(dapp_storage: &DappStorage): u32 {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(TABLE_NAME);
        let _b0 = dapp_system::get_global_field<DappKey>(dapp_storage, key_tuple, b"value");
        let mut _c0 = sui::bcs::new(_b0);
        let value = sui::bcs::peel_u32(&mut _c0);
        value
    }
    ...
    public(package) fun set(dapp_storage: &mut DappStorage, value: u32, ctx: &mut TxContext) {
        ...
        dapp_system::set_global_record<DappKey>(dapp_storage, key_tuple, field_names, value_tuple, OFFCHAIN, ctx);
    }
