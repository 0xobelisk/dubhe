...
## DappKey — Package-Level Authorization Token

### What It Is

`DappKey` is a zero-size struct generated by `dubhe generate` into every DApp package at
`sources/codegen/dapp_key.move`. It serves as a compile-time proof that a call originated
from inside the DApp's own package.

```move
public struct DappKey has copy, drop {}

public(package) fun new(): DappKey { DappKey {} }   // ← THE critical line
```

### Why `new()` Must Stay `public(package)`

Every framework write function that can modify user state requires `_auth: DappKey` as its
first argument:

```move
// set_record, set_field, take_record (list), create_user_storage, … all follow this pattern:
public fun set_record<DappKey: copy + drop>(
    _auth:        DappKey,   // ← caller must supply a live DappKey instance
    user_storage: &mut UserStorage,
    …
)
```

Because `DappKey::new()` is `public(package)`, only bytecode compiled into the DApp's own
package can construct a `DappKey` instance. An external PTB cannot fabricate one.
This means every one of those functions is **automatically package-gated**: the only way
a user can trigger them is through a `public entry fun` that the DApp developer explicitly
provides.

```
External PTB
  → can call any public entry fun in the DApp package
  → those entry funs call dapp_key::new() internally  (public(package) ✅)
  → then pass DappKey instance to the framework function

External PTB
  → cannot call framework write functions directly, because it has no way
    to construct DappKey {}  (public(package) blocks external construction)
```

### What Must Never Be Done

#### 1. Changing `new()` to `public`
...
This allows any PTB to call `dapp_key::new()` directly and pass the result to any
framework write function, bypassing every entry-function guard the DApp provides.

#### 2. Accepting `DappKey` as an external parameter

```move
// ❌ NEVER DO THIS — same effect as making new() public
public entry fun unsafe_write(auth: DappKey, user_storage: &mut UserStorage, …) {
    dapp_system::set_record(auth, user_storage, …);
}
```

An external caller can fabricate the DappKey by calling any other public function in the
package that happens to return one, or by combining it through another vector. The only
safe pattern is to construct it inline:

```move
// ✅ CORRECT — DappKey is constructed and consumed within the same package call
public entry fun safe_write(user_storage: &mut UserStorage, ctx: &mut TxContext) {
...
| ---------------------------- | ----------------------------------------------------------------------- |
| `dapp_key::new()` visibility | Must remain `public(package)` — never change to `public`                |
| DappKey as entry parameter   | Never accept from an external caller                                    |
| DappKey construction site    | Always inline at the framework call site, never stored or passed around |
| Scope                        | One `DappKey` type per DApp package; cannot be reused across packages   |

### Why This Matters for `create_user_storage`

Before this guard was applied to `create_user_storage`, any user could register a
`UserStorage` directly from a PTB, bypassing the DApp's own `register` entry function
(which might enforce a fee, whitelist, or other condition). Adding `_auth: DappKey` to
`create_user_storage` seals that bypass: users must now go through whatever entry function
the DApp provides, giving the DApp full control over registration policy.
