...
> Note: `@0xobelisk/sui-common` provides `defineConfig`, which is the recommended way to define contract metadata for code generation.
...
First, define your contract configuration with `defineConfig`:
...
### Session Cap Flow (Passkey + Delegate Key)

For "register once, delegate key executes without wallet popup" flow, use
session-cap APIs:

1. Main account submits a registration tx that mints `SessionCap` for delegate.
2. Delegate key submits writes with nonce-protected helpers.
3. Written data stays under the owner subject in the session cap.
...
const response = await dubhe.setRecordWithSessionCap({
  ...
  dappHubId,
  dappKeyType: `${APP_PACKAGE_ID}::dapp_key::DappKey`,
  key: [new TextEncoder().encode('score')],
  value: [new Uint8Array([1, 0, 0, 0])],
  sessionRegistryId,
  sessionCapId
});
```
...
### Dubhe Channel (Real-time API)

The Sui client also includes channel helpers for low-latency submit/query/subscribe flows:
...
See [Dubhe Channel](/dubhe/sui/channel) for end-to-end examples.
