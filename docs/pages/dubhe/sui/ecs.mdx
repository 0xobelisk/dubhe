...
1. A running GraphQL server backed by indexer data (see [Indexer](/dubhe/sui/indexer) and [GraphQL server](/dubhe/sui/graphql-server)).
2. Project metadata the client can use for table and field awareness (for example from `dubhe convert-json` / your deployment pipeline).
3. A `createDubheGraphqlClient` instance pointed at the HTTP and WebSocket GraphQL endpoints.
...
// Endpoint: use the dubhe-indexer proxy (http://localhost:8080/graphql) for most
// setups, or the standalone @0xobelisk/graphql-server (http://localhost:4000/graphql).
// See the GraphQL Client page for how to choose.
