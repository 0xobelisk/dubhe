...
> Note: Some config snippets below use legacy style (`DubheConfig` + `schemas`).
> In current Dubhe projects, prefer `defineConfig` + `resources`.
> The migration concepts in this tutorial still apply.
...
🚀 Starting Contract Publication...
  ├─ Project: <project>/packages/contracts/src/dubhe
  ├─ Network: localnet
...
Update deploy log: <project>/packages/contracts/src/dubhe/.history/sui_localnet/latest.json

✅ Dubhe Framework deployed successfully
Updated Dubhe dependency in <project>/packages/contracts/src/counter/Move.toml for localnet.

🚀 Starting Contract Publication...
  ├─ Project: <project>/packages/contracts/src/counter
  ├─ Network: localnet
  ├─ ChainId: 90dbabed
...
Update deploy log: <project>/packages/contracts/src/counter/.history/sui_localnet/latest.json
