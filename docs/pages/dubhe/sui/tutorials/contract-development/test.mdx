...
> ⚠️ **Legacy (Dubhe v1).** This tutorial uses the old `schemas` / `deploy_dapp_for_testing` test bootstrap. For current v2 testing (with `create_user_storage_for_testing` etc.), see [Testing](/dubhe/sui/contracts/testing). Kept for reference pending a rewrite.
