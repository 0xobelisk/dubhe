...
> ⚠️ **Legacy (Dubhe v1).** This tutorial uses the old `schemas` API. In Dubhe v2 you declare `resources` (per-user `UserStorage` and global `DappStorage`) instead of `schemas`. For the current workflow, start with the [Quick Start](/dubhe/sui/quick-start) and the [Resources config](/dubhe/sui/contracts/resources/config). Kept for reference pending a rewrite.
...
- schemas: Data model in the snippets below. Current projects declare storage under `resources`; see [Resources config](/dubhe/sui/contracts/resources/config).
...
Execute the generate command here.

```bash
➜  dubhe-template-project pnpm dubhe generate
