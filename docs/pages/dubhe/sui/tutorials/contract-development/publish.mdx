...
> ⚠️ **Legacy (Dubhe v1).** This tutorial uses the old `schemas` API and references commands that no longer exist (`dubhe call` / `dubhe query` were removed). For the current publish workflow, see the [CLI reference](/dubhe/sui/cli#publish) and the [Quick Start](/dubhe/sui/quick-start). Kept for reference pending a rewrite.
