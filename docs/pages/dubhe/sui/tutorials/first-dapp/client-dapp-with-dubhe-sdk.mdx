...
> ⚠️ **Legacy (Dubhe v1).** This tutorial predates Dubhe v2 and uses the old `schemas` / `StorageValue` / `schema:gen` API. It no longer matches what `pnpm create dubhe` generates (v2 uses `resources` with `DappStorage` / `UserStorage`). For the current client workflow, see [Client](/dubhe/sui/client) and the [Quick Start](/dubhe/sui/quick-start). This page is kept for reference pending a rewrite.
