...
- [resources]: Data model declarations used by `schemagen` to generate typed Move storage modules.

```bash
pnpm --dir packages/contracts run schemagen
```

The generated project structure looks like this:

```
packages/contracts/src/counter
├── Move.toml
└── sources
    ├── codegen
    │   ├── errors
