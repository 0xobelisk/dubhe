...
> ⚠️ **Legacy (Dubhe v1).** This tutorial uses the old `schemas` API and predates Dubhe v2. The ECS concepts still apply; for the current API see the [ECS reference](/dubhe/sui/ecs). Kept for reference pending a rewrite.
