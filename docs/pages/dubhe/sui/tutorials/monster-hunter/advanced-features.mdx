...
> ⚠️ **Legacy (Dubhe v1).** This tutorial uses the old `schemas` / `SubscriptionKind.Schema` API and predates Dubhe v2. For current real-time queries, see the [ECS](/dubhe/sui/ecs) and [GraphQL Client](/dubhe/sui/graphql-client) references. Kept for reference pending a rewrite.
