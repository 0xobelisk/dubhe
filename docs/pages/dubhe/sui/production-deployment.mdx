...
Create a production server setup:

```javascript
// server.js
const { createServer } = require('@0xobelisk/graphql-server');
...
app.use(cors({
  origin: process.env.ALLOWED_ORIGINS?.split(',') || ['https://your-frontend.vercel.app'],
  credentials: true
}));
...
```javascript
// api/graphql.js
import { createServer } from '@0xobelisk/graphql-server';

export default async function handler(req, res) {
  const server = createServer({
    databaseUrl: process.env.DATABASE_URL,
    enableSubscriptions: false, // Serverless doesn't support WebSockets
  });
