...
dubhe store-config --network testnet --output-ts-path ./deployment.ts
```
...
dubhe store-config --network mainnet --output-ts-path ./deployment.ts
```
...
PACKAGE_ID=0x...        # From deployment.ts PackageId
DAPP_STORAGE_ID=0x...   # From deployment.ts DappStorageId
DAPP_HUB_ID=0x...       # From deployment.ts DappHubId
...
# dubhe-indexer exposes an HTTP proxy on port 8080 (GraphQL + subscription)
# and a gRPC backend on port 8085
EXPOSE 8080 8085

# Health check against the proxy endpoint
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
  CMD curl -f http://localhost:8080/health || exit 1

# Required: --config-json path, --checkpoint-url for the Sui remote checkpoint store.
# DATABASE_URL is read from the environment at runtime.
CMD ["dubhe-indexer", "--config-json", "dubhe.config.json", \
...
services:
  indexer:
    ...
    ports:
      - '8080:8080' # HTTP proxy (GraphQL + subscription)
      - '8085:8085' # gRPC backend
    restart: unless-stopped
    depends_on:
      - postgres
    healthcheck:
...
vercel env add NEXT_PUBLIC_DAPP_STORAGE_ID production
...
NEXT_PUBLIC_DAPP_STORAGE_ID=0x...
NEXT_PUBLIC_DAPP_HUB_ID=0x...
...
export const productionConfig = {
  ...
  dappStorageId: process.env.NEXT_PUBLIC_DAPP_STORAGE_ID!,
  dappHubId: process.env.NEXT_PUBLIC_DAPP_HUB_ID!,
...
For high-throughput networks, consider adjusting `start_checkpoint` in `dubhe.config.json` to resume from a known checkpoint, and running the indexer on dedicated hardware with sufficient CPU and I/O capacity.
...
# Restart from a specific checkpoint: update start_checkpoint in dubhe.config.json,
# then restart with --force to rebuild from that checkpoint
dubhe-indexer --config-json dubhe.config.json --checkpoint-url https://checkpoints.testnet.sui.io --force
```
