---
title: CLI - Dubhe
description: Command-line reference for building, testing, and publishing Dubhe projects on Sui.
---
...
18. [`hello`](#hello): Print the Dubhe banner
19. [`shell`](#shell): Open an interactive Dubhe shell
...
> **Common options.** Network-facing commands (`publish`, `upgrade`, `build`, `faucet`, `check-balance`, `info`, `load-metadata`, `switch-env`, `shell`, `generate`) accept `--rpc-url <URL>` to override the RPC fullnode endpoint for that single invocation (useful for private/custom nodes). The default value of `--network` is the literal `default`, which resolves to your **active Sui CLI environment** at runtime (`faucet` and `switch-env` default to `localnet` instead).

## Commands

### `generate`

Generate Move code from a `dubhe.config.ts` file. See the [resources config and `generate` documentation](./contracts/resources/config) for more details.

> **`schemagen`:** alias for `dubhe generate` (same behavior).

```bash
dubhe generate [--config-path <path>] [--network <network>] [--mode <mode>]

Options:
  --config-path    Path to config file (default: "dubhe.config.ts")
  --network        Target network (mainnet/testnet/devnet/localnet/default)
  --mode           Settlement mode baked into generated code:
                   "user_pays" (default) | "dapp_subsidizes"
```

> `--mode` controls who pays for storage writes in the generated code: `user_pays` settles from the user, `dapp_subsidizes` settles from the DApp credit pool.
...
Use `--bump-version` when you need to force all callers off an old package — for
...
Options:
  --output-ts-path   Output path for the generated TypeScript file (default: "./deployment.ts")
