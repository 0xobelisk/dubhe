...
> Beyond your own resource tables, the schema also exposes the framework system tables (extended storage, marketplace, sessions, DApp runtime/fee state) under `dubhe`-prefixed fields such as `dubheSceneStorages` and `dubheSessions`. See the [Indexer reference](/dubhe/sui/indexer#framework-system-tables) for the full list.
