...
> **Which GraphQL endpoint?** There are two ways to serve the GraphQL API, and they listen on different ports:
>
> - **`dubhe-indexer` built-in proxy — `http://localhost:8080/graphql`** (recommended default). When you run `dubhe-indexer`, GraphQL, gRPC, playground, and health are all served through the proxy on port **8080**. Use this for most setups. See the [Indexer reference](/dubhe/sui/indexer).
> - **Standalone `@0xobelisk/graphql-server` — `http://localhost:4000/graphql`**. An independent PostGraphile server you run separately against the same database, e.g. to scale GraphQL apart from the indexer. See the [GraphQL Server](/dubhe/sui/graphql-server) page.
>
> Both expose the same schema; pick the endpoint that matches how you run the backend. The examples below use `:4000` for the standalone server — change it to `:8080` if you connect to the indexer proxy.
...
const client = createDubheGraphqlClient({
  // Indexer proxy: 'http://localhost:8080/graphql'
  // Standalone graphql-server: 'http://localhost:4000/graphql'
...
## Extended-Storage Queries

Besides the generic table helpers, the client ships typed helpers for the framework's own indexer system tables — the extended storage (`objects` / `scenes`), the marketplace, user storage, and sessions. Each accepts an options object (`{ dappKey?, first?, after?, ... }`) and returns a paginated `Connection<Row>`.
...
| `getSceneStorageFields({ dappKey, sceneId? / sceneIds?, fieldName?, isDeleted? })` | individual field values for scenes (companion `scene_storage_fields` table)           |
| `getMarketplaceListings({ dappKey, seller?, status?, recordType? })`               | marketplace listings (`status`: `'listed' \| 'sold' \| 'cancelled' \| 'expired'`)     |
| `getDubheUserStorages({ dappKey, canonicalOwner? })`                               | per-user storage registrations                                                        |
| `getDubheSessions({ dappKey, canonical?, active? })`                               | active session-key rows (used internally by `useSessionKey.revalidate()`)             |

The complete list of framework system tables (with their PostgreSQL names and raw GraphQL fields) is in the [Indexer reference](/dubhe/sui/indexer#framework-system-tables). Tables without a dedicated helper are queryable via `getAllTables('<dubheGraphqlField>', …)`.
...
A scene's metadata (`getSceneStorages`) and its field values (`getSceneStorageFields`) live in two separate tables, so reading a scene's full state is typically one `getSceneStorages` call followed by a `getSceneStorageFields` call keyed on the returned `sceneId`s.
...
- `getObjectStorages(options?): Promise<Connection<ObjectStorageRow>>` - Query `ObjectStorage<T>` rows (extended storage)
- `getSceneStorages(options?): Promise<Connection<SceneStorageRow>>` - Query `SceneStorage<T>` metadata rows
- `getSceneStorageFields(options?): Promise<Connection<SceneStorageFieldRow>>` - Query scene field values
- `getDubheSessions(options?): Promise<Connection<DubheSessionRow>>` - Query active session-key rows
