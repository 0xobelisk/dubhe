...
Running `schemagen` produces `sources/codegen/error.move`:
...
> `error::player_not_found(level::has(dh, player))` reads as:
...
After editing the `errors` field in `dubhe.config.ts`, re-run schemagen to update `error.move`:

```sh
node node_modules/@0xobelisk/sui-cli/dist/dubhe.js schemagen
```

The `sources/codegen/` directory is fully managed by schemagen. Do not edit `error.move` by hand — your changes will be overwritten.
